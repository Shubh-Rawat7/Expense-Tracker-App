...
export const createOrUpdateWallet = async (
    ...
    try {
        ...
        if (walletData.image) {
            const imageUploadRes = await uploadFileToCloudinary(walletData.image, "wallet");
...
export const deleteWallet = async (walletId: string): Promise<ResponseType> => {
    try {
        ...
        deleteTansactionByWalletID(walletId);
...
export const deleteTansactionByWalletID = async (walletId: string): Promise<ResponseType> => {
    try {
        let hasMoreTransactions = true;
        while(hasMoreTransactions){
            const transactionsQuery = query(collection(firestore, 'transactions'), where('walletId', '==', walletId));
            ...
            transactionSnapshot.forEach((transactionDooc)=> batch.delete(transactionDooc.ref));
